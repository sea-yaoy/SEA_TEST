...
export default async function handler(
  ...
  const { screeningComment, decisionComment, no, status } = req.body
  ...
  const resData = (await executeQuery(
    "UPDATE api_provider SET ScreeningComent = ?, DecisionComment = ?, Result = ? WHERE AppNo = ?",
    [screeningComment, decisionComment, status, no],
  )) as ResultSetHeader
